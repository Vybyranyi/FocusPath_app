...
describe('Button component', () => {
  ...
  it('renders an icon when icon prop is passed', () => {
    ...
    // Icon img is aria-hidden; query by src attribute
  ...
  it('applies correct Tailwind classes based on variant and size', () => {
    render(<Button type="outline" size="small">Outline Small</Button>);
    const button = screen.getByRole('button', { name: 'Outline Small' });
    expect(button.className).toContain('ring-card');   // outline ring
    expect(button.className).toContain('py-2');        // small padding
    expect(button.className).toContain('rounded-[18px]'); // small radius
  });
});
