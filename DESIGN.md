...
### Чому один origin, а не два сервіси

Це рішення продиктоване сесією. Токени живуть у cookie, а cookie перестають
бути простими щойно клієнт і API опиняються на різних сайтах:

- знадобився б `SameSite=none`, а це третьосторонні cookie, які Safari блокує
  за замовчуванням, а Chrome обмежує;
- знадобився б спільний батьківський домен, тобто **власний домен** — на
  безкоштовних піддоменах хостингів (`*.vercel.app`, `*.onrender.com`) це
  неможливо, бо вони в Public Suffix List;
- CSRF-токен із другої лінії захисту перетворився б на єдину.

Один origin знімає все це разом: cookie першосторонні, `SameSite=lax` працює
як є, CORS не задіяний, а CSRF лишається тим, чим його спроєктували.
...
SPA-фолбек, навпаки, стоїть **після маршрутів** і має два запобіжники:
...
2. **Відповідає лише тим, хто просить `text/html`.** Браузер при навігації це
   робить завжди, а `fetch` і XHR шлють `*/*`. Без цієї умови поведінка
   застосунку залежала б від того, чи лежить поруч збірка клієнта — і тести
   давали б різний результат локально та в CI.

`/healthz` існує саме через друге правило: health-checker шле `*/*` і отримав
би 404 замість підтвердження, що деплой живий.
