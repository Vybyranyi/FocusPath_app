...
| CSRF-токен | **ні** | `/` | 7 днів | Клієнт повторює його в заголовку |
...
CSRF-cookie має дві назви. У продакшені — `__Host-csrf_token`; префікс `__Host-`
означає, що встановити її здатен лише цей самий хост, лише через HTTPS, лише з
`Path=/` і без `Domain`. Це важливо саме тут: перевірка порівнює cookie із
заголовком і більше нічим, тож хто може підкинути цю cookie — може вибрати й
заголовок, а сусідній субдомен здатен ставити cookie на батьківський домен.
Префікс прибирає цю можливість. Він потребує `Secure`, тому локально назва
лишається `csrf_token`, і забороняє `Domain`, тому неможливий у деплої з
`COOKIE_DOMAIN`. Стару назву сервер приймає й далі — інакше вже видані сесії
не пройшли б перевірку навіть на `/auth/refresh` і не мали б як оновитися.
...
прочитати**. Тому CSRF-токен видається читабельним: клієнт зобов'язаний
...
**Формат дня на дроті — `YYYY-MM-DD`.** Календарний день — це локальний день
користувача; на дріт він їде голим ключем, ніколи повним ISO-моментом. Клієнт
має дзеркальний модуль [`lib/dates.ts`](frontend/src/lib/dates.ts), який це
тримає в одному місці. Історія тут повчальна: клієнт цього контракту спершу не
дотримувався і слав `toISOString()` від локальної півночі — а це попередній день
за UTC на схід від Гринвіча. Одна ця конверсія давала три різні на вигляд баги:
щойно створена звичка «зникала» при поверненні на сьогоднішній день, сусідні дні
показували спільний стан (клітинка «завтра» приносила й відмічала запис
«сьогодні»), а вибір сьогоднішньої дати у формі відхилявся як «дата в минулому».
Зворотний бік того самого правила: мітки з відповіді читаються UTC-акцесорами —
локальні зсувають їх на добу на захід від Гринвіча.

**Стан дня — enum, а не булеве значення.** `pending | done | failed`
([`shared/src/habit.d.ts`](shared/src/habit.d.ts)). `failed` — це користувач, що
каже «я цього не зробив»; булеве значення такого сказати не могло, бо і воно, і
«день ще не настав» були `false`. Наслідок був видимий: свайп вліво фарбував
картку червоним, але цей висновок жив лише в локальному стані компонента і гинув
на першому ж перезапиті.

**«Прострочено» виводиться, а не зберігається.** День, що лишився `pending`,
коли він уже минув, — прострочений; це випливає з дати. Зберігати його означало
б процес, який опівночі — у часовому поясі *кожного* користувача, бо саме тоді
закінчується *їхній* день — переписує рядки, і кожен рядок був би хибним у
проміжку між зміною доби і запуском процесу. Виведення безкоштовне і не буває
застарілим ([`lib/habitStatus.ts`](frontend/src/lib/habitStatus.ts)).
...
- серія — це безперервний ряд днів зі станом `done`, що закінчується
  найсвіжішим;
...
- але поблажливість стосується лише дня, щодо якого рішення ще не ухвалене.
  Явний `failed` на сьогодні — це вже рішення, і воно обриває серію;
- ряд, що обірвався раніше за вчора, дає нуль;
- пропущений день (`pending`, який уже минув) обриває ряд так само, як `failed`;
- майбутні дні ігноруються — відмітка дня, до якого життя ще не дійшло, не
  подовжує серію.

Функція йде **назад від сьогодні**, а не збирає виконані дні й міряє проміжки:
цікаві випадки — саме ті, де нічого немає (пропущений день) або де стоїть явна
відмова, а множина виконаних дат не бачить ні того, ні іншого.
...
**Сесія не зберігається на клієнті.** При старті `App` питає `GET /auth/me` —
cookie браузер надішле сам. Відмова для гостя тут нормальний шлях, а не
помилка.

**Але гостя спершу впізнають без запиту.** `hasSessionCookie()` перевіряє
наявність CSRF-cookie — єдиної з трьох, читабельної для JS (клієнт пробує обидві
її назви: спершу `__Host-csrf_token`, потім `csrf_token`). Усі три
видаються й очищаються разом, а ця має точно такий самий строк життя, як
refresh-токен, тож її відсутність означає, що сесії немає взагалі.

Це прибирає два марні запити на кожне відкриття сторінки гостем: `GET /auth/me`,
який міг відповісти лише 401, і `POST /auth/refresh`, який автоматично йшов
слідом, намагаючись оновити те, чого не існувало. **Браузер пише кожну 4xx у
консоль**, тому кожен гість бачив там дві червоні помилки — а шум у консолі
рівно так і привчає не помічати справжніх помилок.

Той самий сигнал гасить і автоматичне оновлення сесії в `apiRequest`: 401 без
сесійної cookie означає «увійдіть», а не «сесія протухла». Це дзеркалить логіку
сервера в `csrf.ts` — запит без сесійної cookie не має сесії, на якій можна
проїхатися.
